...
import { statusBadge, DetailModal } from '../components/SharedComponents'
...
const VolunteerDashboard = () => {
    ...
    const handleAction = async (issueId, action) => {
        if (action === 'status') {
            const complaint = issues.find(i => i._id === issueId)
    ...
    const handleUpdate = async (updatedIssue, type, comment) => {
        if (type === 'comment') {
            try {
                const { data } = await api.post(`/issues/${updatedIssue._id}/comment`, { text: comment });
                const newIssue = { ...updatedIssue, comments: [...(updatedIssue.comments || []), { ...data.comment, userName: user?.name }] };
                setIssues(prev => prev.map(iss => iss._id === updatedIssue._id ? newIssue : iss));
        ...
        // General update
        setIssues(prev => prev.map(iss => iss._id === updatedIssue._id ? updatedIssue : iss));
